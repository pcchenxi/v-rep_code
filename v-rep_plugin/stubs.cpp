...
////////////////////////
        simRegisterScriptCallbackFunctionE("simExtOMPL_setMotionValidationCallback@OMPL", "number result=simExtOMPL_setMotionValidationCallback(number taskHandle,string callback)", setMotionValidationCallback_callback);
////////////////////////////////
...
setMotionValidationCallback_in::setMotionValidationCallback_in()
{
}

setMotionValidationCallback_out::setMotionValidationCallback_out()
{
    result = 0;
}

void setMotionValidationCallback(SScriptCallBack *p, setMotionValidationCallback_in *in_args, setMotionValidationCallback_out *out_args)
{
...
{
    const char *cmd = "simExtOMPL_setMotionValidationCallback";
    ...
    try
    {
        ...
        setMotionValidationCallback(p, cmd, &in_args, &out_args);
    }
    catch(exception& e)
    {
        simSetLastError(cmd, e.what());
...
motionValidationCallback_in::motionValidationCallback_in()
{
}

motionValidationCallback_out::motionValidationCallback_out()
{
}

bool motionValidationCallback(simInt scriptId, const char *func, motionValidationCallback_in *in_args, motionValidationCallback_out *out_args)
{
    int stackID = -1;
...
////////////////
        try
        {
            // write input argument 1 (state) of type array of std::vector<float>
            simPushTableOntoStackE(stackID);
            for(int i = 0; i < in_args->state_tree.size(); i++)
            {
                write__int(i + 1, stackID);
                write__float(in_args->state_tree[i], stackID);
                simInsertDataIntoStackTableE(stackID);
            }
        }
        catch(std::exception& ex)
        ...
        try
        {
            // write input argument 2 (state) of type array of std::vector<float>
            simPushTableOntoStackE(stackID);
            for(int i = 0; i < in_args->state.size(); i++)
            {
                write__int(i + 1, stackID);
        ...
        {
            std::string msg = ex.what();
            msg += " when writing input argument 2 (state)";
            throw exception(msg);
        }

        try
        {
            // write input argument 3 (motion result) of type bool
            write__bool(in_args->valid, stackID);
        }
        catch(std::exception& ex)
        {
            std::string msg = ex.what();
            msg += " when writing input argument 3 (valid)";
            throw exception(msg);
        }
///////////////
