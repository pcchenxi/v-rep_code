...
function OMPL_Processor:init_task(start_name, target_name, task_id)
    ...
    if self.use_validation_callback then
        ...
        _callback_foot_hds = self.foot_hds
    ...
    -- target pose --
    local goalpose= simGetObjectPosition(target_hd, -1) --get_robot_pose(target_hd, joint_hds, self.robot_dim, self.joint_dim)
...
function OMPL_Processor:get_pose(robot_hd)
    local state = simGetObjectPosition(robot_hd, -1)
    local foot_pos = simGetObjectPosition(self.foot_hds[1], robot_hd)

    state[#state+1] = foot_pos[1]
    return state
end
...
function OMPL_Processor:init_statespace(robot_hd, joint_hds, start_pose, goal_pose)
    ...
    state_spaces[2]=simExtOMPL_createStateSpace('feet_space',sim_ompl_statespacetype_joint_position,self.foot_hds[1], {0.08}, {0.15}, 2)               -- base space        
...
function OMPL_Processor:apply_path()
    ...
    for i=1, #self.path-self.state_dim, self.state_dim do
        ...
        render_pose(state, self.foot_hds)
